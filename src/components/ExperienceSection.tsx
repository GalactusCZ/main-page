import { ui, workExperience } from "@/lib/data";
import TimelineItem from "./TimelineItem";
import { Icon } from "@iconify/react";
import { motion } from "framer-motion";
import MotionWrapper from "./MotionWrapper";

export default function ExperienceSection({ lang }: { lang: 'cs' | 'en' }) {
  const data = workExperience.map(job => job[lang]);
              ...
              >
                <div className="flex items-center mb-3">
                  <div className="h-6 w-6 flex items-center justify-center rounded-full bg-purple-500/10 mr-2">
                    <Icon icon="lucide:briefcase" className="h-4 w-4 text-purple-500" />
                  </div>
                  <h4 className="text-sm font-medium">{uiData.keyPoints}</h4>
                </div>
